...
import {
  approveContactConfirm,
  rejectContactConfirm,
} from "@/app/(dashboard)/approvals/actions";
import { contactConfirmStatusLabel } from "@/lib/customers/contact-confirm-status";
import { format } from "date-fns";
import type { FollowUpConfirmStatus } from "@prisma/client";

export type ContactConfirmApproval = {
  id: string;
...
type Props = {
  items: ContactConfirmApproval[];
  showActions?: boolean;
};

export function ContactConfirmApprovalList({ items, showActions }: Props) {
  if (items.length === 0) {
        ...
        <li key={row.id} className="rounded-md border p-4 text-sm">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <span className="rounded bg-muted px-2 py-0.5 text-xs">
              {APPROVAL_TYPE_LABELS.CONTACT_CONFIRM}
            </span>
            <span className="font-medium">{row.createdBy?.name ?? "未知"}</span>
          ...
          <p className="text-muted-foreground">
            状态：{contactConfirmStatusLabel(row.confirmStatus)}
            {row.title ? ` · ${row.title}` : ""}
            {row.phone ? ` · ${row.phone}` : ""}
          ...
          {showActions && row.confirmStatus === "PENDING_MANAGER" ? (
            <div className="mt-3 flex flex-wrap gap-2">
              <form action={approveContactConfirm}>
                <input type="hidden" name="contactId" value={row.id} />
                <Button type="submit" size="sm">
                  确认入库
                </Button>
              </form>
              <form action={rejectContactConfirm} className="flex flex-wrap items-center gap-2">
                <input type="hidden" name="contactId" value={row.id} />
