...
import { isMobileTaskAction } from "@/lib/mobile/task-actions";
import { cn } from "@/lib/utils";

function mobileHrefForTask(item: {
  kind: string;
...
export default async function MobileTasksPage() {
  ...
  const now = new Date();
  const { items, week } = await listUpcomingActionsThisWeek(session.user.role, userId, 50);
  const tasks = items.filter(isMobileTaskAction);
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <header className="shrink-0 border-b bg-card px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <Link href="/mobile" className="text-xs text-primary">
          ← 今日
        </Link>
        <h1 className="mt-1 text-lg font-bold">待办任务</h1>
        <p className="text-xs text-muted-foreground">
          本周 {weekLabel} · 指派任务与催收回款（完成前首页会提示数量）
        </p>
      </header>

      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain p-4 pb-8">
        {tasks.length === 0 ? (
          <p className="py-10 text-center text-sm text-muted-foreground">本周暂无待办任务</p>
        ) : (
          <ul className="space-y-2">
            {tasks.map((item) => {
              const remaining = getRemainingTimeInfo(item.dueAt, now);
              const href = mobileHrefForTask(item);
              const canMarkDone =
